				...
				<div v-for="(l,index) in chatLists" :key="index" style="margin-bottom:2rem" :class="(index == chatLists.length-1)?'active':''">
					...
					<div class="chatStyle">
						...
						<div v-if="l.type == 2">
							<div class="friend" v-if="l.direction == 1">
								<img :src="l.contactAvatarUrl" class="touxiang" />
								<img :src="l.content" style="width:100px" @click="openImg">
							</div>
							<div class="myself" v-if="l.direction == 2">
								<img :src="l.ownerAvatarUrl" class="touxiang" />
	...
	export default {
		...
		methods: {
			...
			init(){
				let obj = {}
				let _this = this
				this.ws = new WebSocket(this.websocketurl + "/chat/binarySocketServer?userId="+3);
				...
				_this.$refs.input.focus();
				  ...
				  _this.ws.onerror = function(evnt) {
					console.log("fail");
				};    
		        _this.ws.onclose = function(evnt) {
			...
			sendBtn() {
				...
				let resObj = {
				 ...
				 this.ws.send(new Blob([1,resObj]));
				 console.log(new Blob([1,resObj]))
//				obj = {
//					direction: 2,
//					id: 1,
//					type: 1,
